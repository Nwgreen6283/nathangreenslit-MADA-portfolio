...
Data cleaning involved making a new column in the `egg` data set "egg_hen" which represents the number of eggs per hen. Variables within columns were also renamed for convienience.
...
Final cleaning was conducted (eliminating unneeded columns) and the data was split into training and test data (70:30). A null linear model was run for comparison of metrics. Four models were tested in this exercise:
...
Metrics were run (Primarily RMSE) and compared to the Null model to assess performance. Additionally, visualization methods were used in conjunjtion with the performance metrics.

#### Conclusion

Based on the metrics, the question, and the plots, the ANOVA model was chosen, and again run but on the test data. Ultimately, it had a higher RMSE than the train data, but still lower than the null ANOVA.
